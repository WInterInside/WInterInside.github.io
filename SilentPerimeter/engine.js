...
const Zone=(()=>{
 ...
 class Expedition{
  constructor(seed=Date.now(),run=1){
   this.s={version:3,artifactPowerVersion:2,seed:seed>>>0,randomState:seed>>>0,run,mode:'camp',hp:5,weapon:0,armor:0,artifacts:[],medkits:0,money:0,key:false,secretMap:false,elapsed:0,distance:0,steps:0,facing:1,encounters:0,escaped:0,visited:[],log:[],restTime:0,weatherTimer:0,groupSound:null,night:false,event:null,pending:null,previousMode:null,gateBlocked:false,outcome:null};
   this.s.sector=this.integer(1,100);this.s.night=this.roll()<B.nightChance;
   this.s.weapon=this.integer(0,2);this.s.weather=this.pick(['wind','rain','fog','clear']);this.s.weatherTimer=this.integer(45,80);
  ...
  mapPosition(map,type,random=()=>this.roll(),ignore=null){
   const isBuilding=p=>!['anomaly','corpse','cache','wallet','backpack','tractor','tank','helicopter','crashSite','fighter'].includes(p.type);
  ...
  generateMap(){
   ...
   const types=['house','house','house','factory','factory','bunker','dugout','camp','waterTower','cemetery','laboratory','cache','cache','cache','corpse','corpse','corpse','wallet','wallet','cache','hospital','weatherStation','radar'];
   const names={hospital:['Заброшенная больница'],weatherStation:['Метеостанция'],radar:['Радиолокационный комплекс'],house:['Дом лесника','Пустая изба','Дом у просеки','Сгоревший хутор'],factory:['Завод «Рассвет»','Насосная станция','Ремонтный цех'],bunker:['Глухой бункер','Бункер без номера'],dugout:['Землянка под корнями','Чужое убежище'],camp:['Заброшенный лагерь','Стоянка без людей'],waterTower:['Старая водонапорная башня'],cemetery:['Старое кладбище'],laboratory:['Закрытая лаборатория'],cache:['Тайник под камнями','Ржавый контейнер','Свёрток в корнях','Скрытый ящик'],corpse:['Тело у дерева','Незнакомец в плаще','Тело с блокнотом'],wallet:['Потерянный кошелёк','Забытая сумка']};
   const counters={};for(const type of types){const p=placement(),n=counters[type]||0;counters[type]=n+1;map.pois.push({...p,id:'p'+map.pois.length,type,name:names[type][(n+this.s.sector)%names[type].length],description:this.text(type),variant:this.integer(0,3),searched:false})}
   ...
   this.populateGroups(map);map.groupLayoutVersion=2;
   return map;
  }
  populateGroups(map){
   while(map.groups.length<8){const i=map.groups.length,type=map.groups.filter(g=>g.type==='dogs').length<4?'dogs':'boars';let p=null;
    ...
    for(let tries=0;tries<12000;tries++){const candidate=tries<1500?{x:clamp(region.x+this.integer(-170,170),80,map.width-80),y:clamp(region.y+this.integer(-180,180),90,map.height-90)}:{x:this.integer(80,map.width-80),y:this.integer(90,map.height-90)};
     if(distance(candidate,map.camp)<=B.world.groupCampDistance||distance(candidate,map.exit)<=150||map.pois.some(q=>distance(candidate,q)<=65)||map.groups.some(q=>distance(candidate,q)<=B.world.groupSeparation)||(this.s.map&&distance(candidate,this.s)<220))continue;p=candidate;break}
  ...
  spreadGroups(){
   ...
   for(let attempt=0;attempt<30;attempt++){const placed=[];let complete=true;
    for(const group of groups){let chosen=null;
     for(let tries=0;tries<12000;tries++){const p=tries===0&&attempt===0||group.id===pendingId?{x:group.x,y:group.y}:{x:this.integer(80,map.width-80),y:this.integer(90,map.height-90)};
      if(group.id===pendingId){chosen=p;break}
      if(distance(p,map.camp)<=B.world.groupCampDistance||distance(p,map.exit)<=150||map.pois.some(q=>distance(p,q)<=65)||placed.some(q=>distance(p,q)<=B.world.groupSeparation)||distance(p,this.s)<220)continue;chosen=p;break}
     if(!chosen){complete=false;break}placed.push({...chosen,group});
    }
    if(complete){for(const p of placed)Object.assign(p.group,{x:p.x,y:p.y,home:{x:p.x,y:p.y},target:{x:p.x,y:p.y},nextTurn:0});map.groupLayoutVersion=2;return}
   }
   throw Error('Не удалось разнести группы мутантов');
  }
  artifact(rare=null){const type=this.pick(['stamina','speed','preparation']),range=B.artifacts[rare?'enhanced':'normal'][type];return{id:'a'+this.integer(1,9999999),type,value:2*this.integer(...range),name:rare?this.pick(B.enhancedNames)+' '+(rare==='chimera'?'Химеры':'Кровопийцы'):this.pick(B.artifactNames),sprite:this.integer(0,5),rare:!!rare}}
  artifactLabel(a){return a.type==='stamina'?`+${a.value} с выносливости`:a.type==='speed'?`+${a.value}% скорости`:`+${a.value}% подготовки`}
  receive(lines,artifact=null,title='Находки осмотрены'){
   if(artifact&&this.s.artifacts.length>=B.artifacts.slots){this.s.pendingArtifact={artifact,lines,title};this.notice('Пояс заполнен',`Найден «${artifact.name}»: ${this.artifactLabel(artifact)}. Выбери замену или оставь находку.`,lines.join('\n'),this.s.artifacts.map((a,i)=>({id:'replaceArtifact',value:i,label:`${i+1}. Заменить «${a.name}» · ${this.artifactLabel(a)}`})).concat({id:'leaveArtifact',label:'Оставить находку'}),'АРТЕФАКТ');return}
  ...
  gear(laboratory=false){const s=this.s;if(laboratory){const armor=B.armor.length-1;if(s.armor<armor){s.armor=armor;return B.armor[armor].name+' получен.'}return 'Броня уступает твоему снаряжению.'}
   if((this.roll()<.5||s.armor>=4)&&s.weapon<7){s.weapon++;return 'Оружие: '+B.weapons[s.weapon].name+'.'}if(s.armor<4){s.armor++;return 'Броня: '+B.armor[s.armor].name+'.'}return 'Твоё снаряжение лучше находки.';
  }
  revealTunnel(){
   const s=this.s,map=s.map;if(!s.secretMap||map.pois.some(p=>p.type==='tunnel'))return;
   const candidates=[];for(let y=55;y<=175;y+=20)for(let x=80;x<map.width-80;x+=20){const p={x,y};if(distance(p,map.exit)>190&&map.pois.every(q=>distance(p,q)>100))candidates.push(p)}
  ...
  findNote(source){
   this.initializeNotes();
   const type=({zombies:'corpse',hospital:'house',weatherStation:'waterTower',radar:'factory'})[source?.type]||source?.type;
   ...
   const placeSetting={hospital:'В больничной палате найдена записка',weatherStation:'В аппаратной метеостанции найдена записка',radar:'В рубке радиолокационного комплекса найдена записка'}[source?.type]||setting;
   return [`${placeSetting}.\n${note.body.replace(/\n\s*\n/g,'\n')}`];
  }
  rareItems({map=false,key=true,source=null}={}){const lines=[];if(key&&!this.s.key&&this.roll()<B.loot.key){this.s.key=true;lines.push('Найдена ключ-карта от секретной лаборатории.')}if(map&&!this.s.secretMap&&this.roll()<B.loot.secretMap){this.s.secretMap=true;this.revealTunnel();lines.push(this.secretMapStory(source))}return lines}
  poiLoot(p,initial=[]){const lines=[...initial,this.money()];if(this.roll()<.55)lines.push(this.gear());if(this.s.medkits<2&&this.roll()<B.loot.medkit){this.s.medkits++;lines.push('Найдена 1 аптечка.')}const special=['bunker','dugout','camp','waterTower','corpse','cache','cemetery','hospital','weatherStation','radar'].includes(p.type);
   if(p.type==='corpse'||this.roll()<.30)lines.push(...this.findNote(p));lines.push(...this.rareItems({map:special,source:p}));this.receive(lines,null,p.name);
  }
  explore(id){const p=this.s.map.pois.find(p=>p.id===id);if(!p)return;
   ...
   if(p.type==='laboratory'&&!this.s.key){this.notice(p.name,p.description,'Закрыто. Нужен специальный ключ. Шанс находки ключа при подходящем исследовании — 5%.');return}
  ...
  search(){const p=this.s.map.pois.find(p=>p.id===this.s.pending?.id);if(!p||p.searched)return;this.s.pending=null;p.searched=true;this.s.visited.push(p.id);
   if(p.type==='laboratory'){const reward=this.integer(0,2);const lines=['Лаборатория открыта и исследована.'];if(reward===0){lines.push(this.gear(true));this.receive(lines)}else if(reward===1){if(this.s.weapon<8){this.s.weapon=8;lines.push('Найдено оружие Гаусса.')}else lines.push('Оружие Гаусса уже есть.');this.receive(lines)}else this.receive(lines,this.artifact());return}
   if(['corpse','cache','wallet','backpack'].includes(p.type)){this.poiLoot(p);return}
  ...
  takeAnomaly(){const pending=this.s.pending;if(!pending||pending.kind!=='anomaly')return;this.s.pending=null;const p=this.s.map.pois.find(p=>p.id===pending.id);if(p?.searched){this.notice(p.name,'Здесь больше нечего искать. Ты уже проверил каждый угол.','Исследована');return}if(p){p.searched=true;this.s.visited.push(p.id)}this.s.encounters++;
   const success=this.roll()*100<this.anomalyChance();
   if(!success&&this.roll()<1/3){this.s.hp=0;this.finish('defeat','Аномалия «'+pending.name+'» сомкнулась. Выбраться не удалось.');return}
   ...
   if(!success){const message=`Ты с трудом выбрался из аномалии. Потеряно ${damage} секц. здоровья. Артефакт остался внутри.`;this.log(message);this.notice('Удалось выжить',message,'');return}
  ...
  randomEvent(){
   ...
   const r=this.roll();if(r<.10)this.encounter('dogs');else if(r<.20)this.encounter('boars');else if(r<.49)this.encounter('bandits');else if(r<.76){const element=this.pick(['acid','electric','fire','gravity']);this.anomaly({name:this.pick(B.anomalyFirst)+' '+this.pick(B.anomalySecond),element,description:this.text(element)})}else{const lines=[this.text('trail'),this.money(),...this.rareItems({map:true,source:{type:'trail'}})];this.receive(lines,null,'Следы на тропе')}
  }
  nearbyGroup(type){
  ...
  encounter(type,extra={}){const group=extra.groupId?null:!extra.poiId&&this.nearbyGroup(type);if(group){extra={...extra,groupId:group.id};group.cooldown=B.encounters.groupCooldown;group.nextTurn=0}this.s.pending={kind:'encounter',type,...extra};const names={dogs:'Стая слепых псов',boars:'Группа кабанов',indoorDog:'Собака в помещении',bandits:'Засада',bloodsucker:'Кровопийца',chimera:'Химера',zombies:'Фигуры среди могил',trap:'Ловушка',infected:'Заражённый участок',unknown:'Неизвестное существо'};this.s.soundCue=type;
   this.notice(names[type],this.text(type),`Пройти без ранений: ${Math.round(this.combatChance(type))}%. Отступить: ${this.escapeChance(type)}%.`,[{id:'resolve',label:'Пройти встречу'},{id:'escape',label:'Попытаться отступить'}],['bloodsucker','chimera'].includes(type)?'ОСОБАЯ УГРОЗА':'ВСТРЕЧА');
  }
  ...
  moveGroups(dt){for(const group of this.s.map.groups){
   ...
   if(group.nextTurn<=0||distance(group,group.target)<10){group.target={x:clamp(group.home.x+this.integer(-190,190),70,B.world.width-70),y:clamp(group.home.y+this.integer(-170,170),70,B.world.height-70)};group.nextTurn=this.integer(7,17)}
   const d=distance(group,group.target),speed=group.type==='dogs'?15:10;if(d>1){const next={x:group.x+(group.target.x-group.x)/d*speed*dt,y:group.y+(group.target.y-group.y)/d*speed*dt};const safe=distance(next,this.s.map.camp)>B.world.groupCampDistance&&distance(next,this.s.map.exit)>130&&this.s.map.groups.every(q=>q===group||q.defeated||distance(q,next)>B.world.groupMovingSeparation)&&this.s.map.pois.every(p=>distance(p,next)>55);if(safe){group.x=next.x;group.y=next.y}else group.nextTurn=0}
  ...
  update(dt,dx=0,dy=0){dt=clamp(dt,0,.1);const s=this.s;
   ...
   const length=Math.hypot(dx,dy);if(length>0){const ox=s.x,oy=s.y;const step=Math.min(dt,s.stamina);s.x=clamp(s.x+dx/length*this.speed()*step,18,B.world.width-18);s.y=clamp(s.y+dy/length*this.speed()*step,18,B.world.height-18);const moved=distance(s,{x:ox,y:oy});if(moved>0){s.distance+=moved;s.steps+=moved;s.stamina=Math.max(0,s.stamina-step);if(dx)s.facing=Math.sign(dx)}
  ...
  upgradeArtifacts(){
   const s=this.s;if(s.artifactPowerVersion===2)return;
   for(const a of s.artifacts)a.value*=2;
   const pending=s.pendingArtifact;if(pending){pending.artifact.value*=2;
  ...
  upgradeAnomalyLayout(){
   ...
   for(const p of map.pois.filter(q=>q.type==='anomaly'&&!q.searched)){
    ...
    const buildings=map.pois.filter(q=>!['anomaly','corpse','cache','wallet','backpack','tractor','tank','helicopter','crashSite','fighter'].includes(q.type));
  ...
  upgradeWorld(){
   ...
   if(map.width===1600&&map.height===1000){
    map.width=B.world.width;map.height=B.world.height;
    const additions=[['hospital','Заброшенная больница',260],['weatherStation','Метеостанция',590],['radar','Радиолокационный комплекс',920]];
    for(const [type,name,y] of additions){
     const p={id:'expansion-'+type,type,name,x:1750,y,description:B.texts[type][0],variant:0,searched:false};
     const near=[map.camp,...map.pois,map.exit].sort((a,b)=>distance(a,p)-distance(b,p))[0];
     map.pois.push(p);map.roads.push({wide:true,points:[{x:near.x,y:near.y},{x:p.x,y:p.y}]});
    }
    map.forests.push({x:1740,y:420,radius:130},{x:1730,y:1090,radius:135},{x:800,y:1110,radius:160});
   }
  ...
  static restore(data){const s=typeof data==='string'?JSON.parse(data):data;if(!s||s.version!==3||!s.map||!((s.map.width===B.world.width&&s.map.height===B.world.height)||(s.map.width===1600&&s.map.height===1000))||!Array.isArray(s.map.pois)||![4,8].includes(s.map.groups?.length)||!Array.isArray(s.artifacts)||s.artifacts.length>6||!Number.isFinite(s.hp)||s.hp<0||s.hp>5||!Number.isFinite(s.x)||!Number.isFinite(s.y)||!B.weapons[s.weapon]||!B.armor[s.armor])throw Error('Неподходящее сохранение');const e=Object.create(Expedition.prototype);e.s=s;e.upgradeWorld();e.upgradeAnomalyLayout();e.initializeNotes();e.upgradeArtifacts();if(!s.encounterSpacingVersion){s.nextEncounter=s.distance+Math.max(0,s.nextEncounter-s.distance)*2;s.encounterSpacingVersion=2}e.populateGroups(s.map);if(s.map.groupLayoutVersion!==2)e.spreadGroups();e.revealTunnel();if(s.mode==='dialog'&&s.pending?.kind==='anomaly'&&s.event?.tag==='АНОМАЛИЯ')e.anomaly({...s.pending,description:s.event.body});if(s.event?.tag==='ГРАНИЦА')s.event.actions=s.event.actions.filter(a=>a.id!=='secretExit');return e}
 }
